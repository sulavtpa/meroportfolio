const commands = {
  ...
  about: {
    ...
    execute: () =>
      "A developer in my early 20s, continuously evolving, building, and learning through every project and mistake.",
  },
  skills: {
    description: "List technical skills",
    execute: () => {
      return (
        `<span class="terminal-command">Languages:</span> C/C++, Java, Rust, Python, JavaScript\n` +
  ...
  projects: {
    ...
    execute: () =>
      "The portfolio gallery is currently under construction. New projects will be showcased here shortly.",
  },
  clear: {
    ...
    execute: (args, terminalBody) => {
      ...
      terminalBody
        ...
        .addEventListener("keydown", (e) =>
          handleCommand(e, terminalBody, commands),
        );
  ...
  sudo: {
    ...
    execute: () =>
      `<span class="terminal-error">Error: User is not in the sudoers file. This incident will be reported.</span>`,
  },
  date: {
  ...
  calc: {
    description: "Calculate maths expressions",
    execute: (args) => {
      const expression = args.join("");
      if (!expression) return "Usage: calc <expression>";
      try {
        if (/[^-+*/().0-9e\s%^]/.test(expression)) {
          return '<span class="terminal-error">Error: Invalid characters in expression</span>';
        }
        const sanitizedExpression = expression.replace(/\^/g, "**");
        const result = new Function(`return ${sanitizedExpression}`)();
        if (result === Infinity || isNaN(result)) {
          return `<span class="terminal-error">Error: Division by zero or invalid result</span>`;
        }
        return `<span class="terminal-success">${result}</span>`;
      } catch (error) {
  ...
  fastfetch: {
    ...
    execute: () => {
      const lines = [
        ...
        [
          "               .##@o@#####@",
          " <span class='terminal-command'>CPU:</span> Intel i7-14700HX (28) @ 5.500GHz",
        ],
        [
          "              /############@",
        ...
        [
          "           @######`     %#####o",
          " <span class='terminal-command'>DE:</span> GNOME 47",
        ],
        [
          "          @######@       ######%",
        ...
        [
          "        -@#######h       ######@.`",
          " <span class='terminal-command'>Theme:</span> Everforest-Dark-B",
        ],
        [
          "       /#####h**``       `**%@####@",
          " <span class='terminal-command'>Icons:</span> Tela-Circle-Dark",
        ],
        [
          "      @H@*`                    `*%#@",
          " <span class='terminal-command'>Terminal:</span> Kitty",
        ],
        ["     *`                            `*", ""],
      ];
      ...
      let fastfetchOutput =
        '<pre style="font-family: monospace; line-height: 1.4; white-space: pre;">';
      lines.forEach((line) => {
  ...
  socials: {
    ...
    execute: () => {
      return (
        `<span class="terminal-command">LinkedIn:</span> https://www.linkedin.com/in/sulav-thapa-9a51b7376/\n` +
  ...
  if (e.key === "Enter") {
    ...
    if (cmd !== "clear") {
      ...
      newTerminalInput.addEventListener("keydown", (e) =>
        handleCommand(e, terminalBody, commandsObj),
      );
